
C----------------------------------------------------------------------
C FUNCTION STAMP
C
C Called by: LAIAMP
C Calls:     TRINT
C
C Purpose: Estimate amplitude
C
C Formal parameters:
C      Epsi   - epsilon for this experiment
C      Errt   - sqrt(epsilon^2 - 1) for this experiment
C      Xiv    - value of xi
C      Dw     - step in omega (0.03)
C      W0     - value of omega
C      Lmda   - lambda (1...2 for E1...2 and 7 for M1 - other multipolarites forbidden)
C      Mua    - mu + 1
C
C Note that the pre-factors in the fct values correspond to those of the
C collision functions.
C
C Return value:
C      Estimated amplitude

      COMPLEX*16 FUNCTION STAMP(Epsi,Errt,Xiv,Dw,W0,Lmda,Mua)
      IMPLICIT NONE
      REAL*8 a , axi , b , bic , bic2 , bis , bis2 , ca , cb , cia ,
     &       cib , cic , cis , Dw , dwi , Epsi , Errt , ex , exa , fct
      INTEGER*4 la , Lmda , mi , Mua
      REAL*8 sa , sb , sia , sib , W0 , Xiv
      DATA fct/0./

      mi = Mua - 1
      axi = ABS(Xiv) ! Absolute value of xi
      la = Lmda
      IF ( Lmda.EQ.7 ) la = 3

      IF ( axi.LT.1.D-5 ) THEN ! Small absolute values of xi
         a = -2.*W0
         IF ( la.EQ.3 ) a = -W0
         exa = EXP(a)
         dwi = 3.D0*Dw
         cic = exa*(EXP(dwi)-1.D0)
         STAMP = DCMPLX(cic,0.D0)
         IF ( la.EQ.2 ) THEN
           IF ( mi.EQ.0 )
     &       fct = 3.D0*(3.D0-Epsi*Epsi)/Epsi/Epsi/Epsi/Epsi
           IF ( mi.EQ.1 )
     &       fct = 1.837117307D0*Errt/Epsi/Epsi/Epsi/Epsi ! 1.837117307 = 3/2 * sqrt(3/2)
           IF ( mi.EQ.2 )
     &       fct = -3.674234613D0*Errt*Errt/Epsi/Epsi/ ! 3.674234613 = 3 * sqrt(3/2)
     &                           Epsi/Epsi
         ELSEIF ( la.EQ.3 ) THEN
            fct = -1.414213562D0*Errt/Epsi/Epsi ! 1.414213562 = sqrt(2)
         ELSE
            IF ( mi.EQ.0 ) fct = 1.D0/Epsi/Epsi
            IF ( mi.EQ.1 ) fct = 1.414213562D0*Errt/Epsi/Epsi ! 1.414213562 = sqrt(2)
         ENDIF
      ELSE ! Larger absolute values of xi
        ex = EXP(W0)/2.D0
         b = axi*(Epsi*ex+W0)
         CALL TRINT(b,sib,cib)
         sb = SIN(b)/b
         cb = COS(b)/b
         bis = sb + cib
         bic = cb - sib
         bis2 = -sb/b
         bic2 = -cb/b
         dwi = -3.D0*Dw
         exa = EXP(dwi)
         a = axi*(Epsi*ex*exa+W0+dwi)
         sa = SIN(a)/a
         ca = COS(a)/a
         CALL TRINT(a,sia,cia)
         cis = sa + cia - bis
         cic = ca - sia - bic
         IF ( la.EQ.1 ) THEN
            STAMP = DCMPLX(cic,cis)
         ELSE
            dwi = (bic2-cis+ca/a)/2.
            exa = (bis2+cic+sa/a)/2.
            STAMP = DCMPLX(dwi,exa)
         ENDIF
         IF ( la.EQ.2 ) THEN
           IF ( mi.EQ.0 )
     &        fct = .75D0*(3.D0-Epsi*Epsi)*axi*axi/Epsi/Epsi
           IF ( mi.EQ.1 )
     &        fct = 1.837117307D0*Errt*axi*axi/Epsi/Epsi ! 1.837117307 = 3/2 * sqrt(3/2)
           IF ( mi.EQ.2 )
     &        fct = -.9185586535D0*Errt*Errt*axi*axi/Epsi/ ! 0.9185586535 = 3/4 * sqrt(3/2)
     &                           Epsi
         ELSEIF ( la.EQ.3 ) THEN
            fct = -.3535533905D0*Errt*axi*axi ! 0.3535533907 = 1/4 * sqrt(2)
         ELSE
            IF ( mi.EQ.0 ) fct = .5D0*axi/Epsi
            IF ( mi.EQ.1 ) fct = .3535533907D0*Errt*axi/Epsi ! 0.3535533907 = 1/2 * sqrt(1/2)
         ENDIF
      ENDIF

      STAMP = STAMP*fct
      STAMP = CONJG(STAMP)
      END
