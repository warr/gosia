...
qfit.f gamatt.f gcf.f tcexp.f tcabs.f tasin.f tacos.f op_intgi.f op_theo.f openf.f effix.f \
...
single_file: include
	./include $(SRCS) |sed -e "s/LATEST REVISION- JUNE  2006/VERSION: $$(grep %global\ version gosia.spec |cut -d\  -f3)/" > $(SINGLE_FILE)
